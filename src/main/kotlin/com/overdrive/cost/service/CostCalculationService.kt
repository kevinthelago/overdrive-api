...
class CostCalculationService(
    ...
    /** Calculates the total delivered cost for a distribution opportunity. */
    @Cacheable(
    ...
    fun calculate(context: CostCalculationContext): DeliveredCostResult {
        val overrides = resolveOverrides(context)
        val steps = mutableListOf<TraceStep>()

        val freightCost = computeFreight(context, overrides, steps)
        val dutyCost = computeDuty(context, overrides, steps)
        val fees = computeFees(context, overrides, steps)

        val total = (listOf(freightCost.amount, dutyCost.amount) + fees.map { it.amount })
            .reduce(Money::plus)
            .scaled()
        ...
        val trace = explainabilityService.record(
            ...
            inputs = context,
            outputs = mapOf(
                "totalDeliveredCost" to total,
                "freightCost" to freightCost,
                "dutyCost" to dutyCost,
                "fees" to fees
            ),
            steps = steps
        )
    ...
    private fun resolveOverrides(context: CostCalculationContext): Map<String, BigDecimal> {
        val scenarioOverrides: Map<String, BigDecimal> = context.scenarioContextId
            ?.let { id -> scenarioContextResolver.resolve(id) }
            ?.assumptions
            ?.let { it.freightRateOverrides + it.dutyRateOverrides + it.feeOverrides }
            ?: emptyMap()
    ...
    private fun computeFreight(
        ...
        val ratePerKg = overrides["freight.rate.${ctx.carrier}"]
            ?: DEFAULT_FREIGHT_RATES[ctx.transportMode]
            ?: BigDecimal("2.50")
        val weightKg = ctx.weight.toKg().value
        val amount = Money((ratePerKg * weightKg).setScale(4, RoundingMode.HALF_UP))
        steps += TraceStep(
            step = 1,
            description = "Freight cost",
            inputs = mapOf(
                "carrier" to ctx.carrier,
                "mode" to ctx.transportMode.name,
                "weightKg" to weightKg,
                "ratePerKg" to ratePerKg
            ),
            outputs = mapOf("freightCost" to amount.amount),
            rule = "freightCost = weightKg × ratePerKg"
        )
    ...
    private fun computeDuty(
        ...
        val dutyRate = overrides["duty.rate.${ctx.productHsCode}"]
            ?: DUTY_RATES.getOrDefault(ctx.destinationCountry, BigDecimal("0.05"))
        val amount = Money((ctx.declaredCargoValue.amount * dutyRate).setScale(4, RoundingMode.HALF_UP))
        steps += TraceStep(
            step = 2,
            description = "Import duty",
            inputs = mapOf(
                "hsCode" to ctx.productHsCode,
                "destinationCountry" to ctx.destinationCountry,
                "dutyRate" to dutyRate,
                "cargoValue" to ctx.declaredCargoValue.amount
            ),
    ...
    private fun computeFees(
        ...
        val handlingFee = FeeComponent(
            type = "HANDLING",
            description = "Handling and documentation",
            amount = Money((ctx.declaredCargoValue.amount * handlingRate).setScale(4, RoundingMode.HALF_UP))
        )
        steps += TraceStep(
            step = 3,
            description = "Handling fees",
            inputs = mapOf("cargoValue" to ctx.declaredCargoValue.amount, "handlingRate" to handlingRate),
            outputs = mapOf("handlingFee" to handlingFee.amount.amount),
            rule = "handlingFee = cargoValue × handlingRate"
        )
        return listOf(handlingFee)
    }
